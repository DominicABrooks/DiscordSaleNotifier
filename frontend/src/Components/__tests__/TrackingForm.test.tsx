...
describe('TrackingForm', () => {
  ...
  it('marks the input invalid while the URL does not match the Discord webhook format', () => {
    render(<TrackingForm formType="add" onSubmitForm={jest.fn()} />);
  ...
  it('falls back to a default message and logs when verification rejects with a non-error', async () => {
    ...
    try {
      ...
      render(<TrackingForm formType="add" onSubmitForm={onSubmitForm} />);
      fillAndSubmit(VALID_URL);
      await waitFor(() => expect(toastError).toHaveBeenCalledWith('Failed to fetch webhook URL', { position: 'top-right' }));
      expect(errorSpy).toHaveBeenCalledWith('Error fetching webhook URL:', 'Failed to fetch webhook URL');
      expect(onSubmitForm).not.toHaveBeenCalled();
    } finally {
...
describe('TrackingForm extra', () => {
  ...
  it('starts with an empty webhook input', () => {
    render(<TrackingForm formType="add" onSubmitForm={jest.fn()} />);
  ...
  it('logs invalid submissions for diagnostics', async () => {
    const logSpy = jest.spyOn(console, 'log').mockImplementation(() => undefined);
    try {
      render(<TrackingForm formType="add" onSubmitForm={jest.fn()} />);
      fillAndSubmit('not-a-webhook');
      await waitFor(() => expect(toastError).toHaveBeenCalled());
      expect(logSpy).toHaveBeenCalledWith('Invalid URL');
    } finally {
      logSpy.mockRestore();
    }
  });
